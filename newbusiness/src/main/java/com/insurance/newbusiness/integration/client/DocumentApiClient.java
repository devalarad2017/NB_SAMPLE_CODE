...
/**
 * DocumentApiClient — calls DOCUMENT_API.
 *
 * TODO: Skeleton — follow the same pattern as EligibilityApiClient (full example).
 * Step 1: Confirm API contract → fill DocumentRequest and DocumentResponse POJOs
 * Step 2: Add partner_field_mapping rows with target_api = DOCUMENT_API
...
public class DocumentApiClient {
    ...
    private static final String STAGE    = "DOCUMENT_STAGE";
    private static final String API_NAME = "DOCUMENT_API";

    @Value("${api.endpoints.document}")
    private String url;
    ...
    public DocumentResponse call(DocumentRequest request, JourneyContext context) {
        long start = System.currentTimeMillis();
        log.info("[{}] Calling {} | url={}", context.getCorrelationId(), API_NAME, url);
        try {
            DocumentResponse response = restTemplate.postForObject(url, request, DocumentResponse.class);
    ...
    @Recover
    public DocumentResponse recover(ApiCallException ex, DocumentRequest req, JourneyContext ctx) {

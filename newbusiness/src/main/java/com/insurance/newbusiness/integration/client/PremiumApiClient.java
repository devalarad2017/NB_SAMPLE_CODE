...
/**
 * PremiumApiClient — calls PREMIUM_API.
 *
 * TODO: Skeleton — follow the same pattern as EligibilityApiClient (full example).
 * Step 1: Confirm API contract → fill PremiumRequest and PremiumResponse POJOs
 * Step 2: Add partner_field_mapping rows with target_api = PREMIUM_API
 * Step 3: Wire into JourneyOrchestrator in the correct stage position
 */
@Service
public class PremiumApiClient {

    private static final Logger log      = LoggerFactory.getLogger(PremiumApiClient.class);
    private static final String STAGE    = "PREMIUM_STAGE";
    private static final String API_NAME = "PREMIUM_API";

    @Value("${api.endpoints.premium}")
    private String url;
    ...
    public PremiumResponse call(PremiumRequest request, JourneyContext context) {
        long start = System.currentTimeMillis();
        log.info("[{}] Calling {} | url={}", context.getCorrelationId(), API_NAME, url);
        try {
            PremiumResponse response = restTemplate.postForObject(url, request, PremiumResponse.class);
    ...
    @Recover
    public PremiumResponse recover(ApiCallException ex, PremiumRequest req, JourneyContext ctx) {
